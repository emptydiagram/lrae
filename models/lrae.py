...
class LRAENetwork(nn.Module):
    def __init__(self, input_size, num_classes, num_hiddens, hidden_size, beta=0.1, gamma=0.2, activation_fn='tanh'):
        ...
        # error weights
        self.E = nn.ParameterList([nn.Parameter(torch.empty(hidden_size, hidden_size), requires_grad=True) for _ in range(num_hiddens - 1)]
        ...
        for E in self.E:
            if isinstance(E, nn.Parameter):
                nn.init.normal_(E, mean=0.0, std=param_std)
    ...
    def compute_targets(self, y):
        ...
        for k in reversed(range(L-1)):
            # E[k] instead of E[k+1] because we don't pad self.E (E^1 doesnt exist)
            y_lower = self.phi(self.h[k] - self.beta * self.e[k+1] @ self.E[k])
    ...
    def backward(self, x, y):
        ...
        self.E[0].grad = -self.gamma * self.W[1].T
